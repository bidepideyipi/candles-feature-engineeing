...
upper_shadow_ratio_4h, lower_shadow_ratio_4h, shadow_imbalance_4h, body_ratio_4h  # 4小时Pinbar（新增）

### 1D框架（新增）
rsi_14_1d       # 日线动量
atr_1d          # 日线波动率
upper_shadow_ratio_1d, lower_shadow_ratio_1d, shadow_imbalance_1d, body_ratio_1d  # 1天Pinbar（新增）
...
| 时间框架 | 特征数 | 特征列表 |
|---------|--------|---------|
| 1小时基础层 | 13 | close_1h_normalized, volume_1h_normalized, rsi_14_1h, macd_line_1h, macd_signal_1h, macd_histogram_1h, hour_cos, hour_sin, day_of_week, upper_shadow_ratio_1h, lower_shadow_ratio_1h, shadow_imbalance_1h, body_ratio_1h |
| 15分钟高频 | 8 | rsi_14_15m, volume_impulse_15m, macd_line_15m, macd_signal_15m, macd_histogram_15m, atr_15m, stoch_k_15m, stoch_d_15m |
| 4小时中期 | 18 | rsi_14_4h, trend_continuation_4h, macd_line_4h, macd_signal_4h, macd_histogram_4h, atr_4h, adx_4h, plus_di_4h, minus_di_4h, ema_12_4h, ema_26_4h, ema_48_4h, ema_cross_4h_12_26, ema_cross_4h_26_48, upper_shadow_ratio_4h, lower_shadow_ratio_4h, shadow_imbalance_4h, body_ratio_4h |
| 1日线框架 | 9 | rsi_14_1d, atr_1d, bollinger_upper_1d, bollinger_lower_1d, bollinger_position_1d, upper_shadow_ratio_1d, lower_shadow_ratio_1d, shadow_imbalance_1d, body_ratio_1d |
...
- 删除原因：total_shadow_ratio_1h 与 body_ratio_1h 线性相关（total_shadow_ratio_1h = 1 - body_ratio_1h）
...
**添加内容**：
- 4 个 Pinbar 比例特征（与 1H 相同）

**添加位置**：
- `src/feature/feature_4h_creator.py`
- `src/feature/feature_merge.py`

**特征列表**：
1. upper_shadow_ratio_4h
2. lower_shadow_ratio_4h
3. shadow_imbalance_4h
4. body_ratio_4h

**预期效果**：
- 中期趋势信号更稳定（4H 比 1H 更平滑）
- 与 1H Pinbar 特征形成多周期确认
- 提升 4H 时间框架的形态识别能力

### 1D 添加 Pinbar 特征

**添加内容**：
- 4 个 Pinbar 比例特征（与 1H、4H 相同）

**添加位置**：
- `src/feature/feature_1d_creator.py`
- `src/feature/feature_merge.py`

**特征列表**：
1. upper_shadow_ratio_1d
2. lower_shadow_ratio_1d
3. shadow_imbalance_1d
4. body_ratio_1d

**预期效果**：
- 长期趋势形态更清晰（1D 消除噪音）
- 与 1H、4H Pinbar 特征形成完整的多周期体系
- 提升日线级别的关键反转点识别能力
...
3. 整体准确率 ：77.31% 对 5 类分类是不错的水平 
4. 中间类别较弱 ：类别 2（跌）和类别 4（涨）F1-Score 仅 0.70-0.71
5. 样本不均衡 ：类别 3 样本数（951）是类别 5（343）的 2.77 倍
```
### 增加样本数到27901条记录
...
============================================================
准确率: 0.7859
交叉验证准确率: 0.7185 (+/- 0.0065)
训练时间: 2026-02-15T21:18:49.604080
各类别置信度:
  类别 1: 0.7304
  类别 2: 0.5496
  类别 3: 0.5961
...
训练时间: 2026-02-15T22:34:01.870454
各类别置信度:
  类别 1: 0.7408
  类别 2: 0.5711
  类别 3: 0.6263
