...
```python
# 在 feature_merge.py 中添加

# 1. 短期波动率异常
"volatility_15m": np.std([c['close'] for c in candles15m[-12:]]) / np.mean([c['close'] for c in candles15m[-12:]])

# 2. 成交量突增
"volume_spike_1h": candles1H[-1]['volume'] / np.mean([c['volume'] for c in candles1H[-5:]])

# 3. 价格跳空
"gap_1h": (candles1H[-1]['open'] - candles1H[-2]['close']) / candles1H[-2]['close']

# 4. 连续下跌天数
"consecutive_downtrend_4h": 1 if candles4H[-1]['close'] < candles4H[-2]['close'] < candles4H[-3]['close'] else 0
```